...
app = FastAPI(title="SVC-C2-016 Field Service Dispatch Agent")
...
# adapter always ran with config={}, so max_retry/timeout_s etc. silently never
# reached Graph() on this path.
_CONFIG_PATH = Path(__file__).resolve().parents[2] / "config" / "config.yaml"
_config = load_config(str(_CONFIG_PATH)) if _CONFIG_PATH.exists() else {}

# This template has no LLM provider wired in: WorkOrderNode (src/nodes/work_order_node.py)
# already degrades to a deterministic work-order narrative when config["llm"] is None
# (see docs/02_design.md "Node-vs-Tool / LLM split") — that was true before this
# migration too, since the prior server.py never passed a config at all. Leaving
# this unset preserves that behaviour; wiring a real provider is a separate decision.
_config.setdefault("llm", None)
...
# This template has neither (propagate_hitl = False, no interrupt()), so this is a
# no-op today, kept for parity with the registry-compiled path.
...
agent.provision_secrets(secrets_factory(namespace="svc", agent_name="svc-c2-016"))
...
async def invoke(req: InvokeRequest, request: Request) -> dict[str, Any]:
    ...
    # ctx.secrets cannot apply. See the framework's security-model documentation
    # and the operation guide for details.
...
def health() -> dict[str, str]:
    return {"status": "ok", "agent": "svc-c2-016"}
